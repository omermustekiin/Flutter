...
Ücretsiz bir API servisi tarafından alınan hizmet ile uygulama içerisinde farklı şehirlere ait hava durumu bilgilerinin sunulduğu, http paketi kullanılarak bir API'dan veriler çekilip işlenerek kullanıcıya sunulduğu bir hava durumu uygulamasıdır. Ayrıca geolocator paketi ile kullanıcının anlık konum bilgisi uygulama başlatılırken alınarak kullanıcının bulunduğu konuma göre hava durumu ve şehir bilgisi ile beraber 5 günlük hava durumu bilgisi de arayüzde sunulur.
